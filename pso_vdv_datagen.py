# -*- coding: utf-8 -*-
"""
Created on Thu Jun 19 11:36:56 2025
...
class AdaptivePSO:
    ...
    def optimize(self, verbose=False):
        ...
        while generations_since_expansion < self.num_iterations and total_generations_done < max_total_generations and generations_since_improvement < max_gen_without_improvement:
...
def doublet_test(kp_dt, ki_dt, kd_dt):
    # Simulation time
    num_sec = 5
    num_hours = 4
...
def run_PID_PSO_test(Vdot_Vr_1, Qk_1, Cain_1, Tin_1, zoom=True, verbose=False):
    ...
    def evaluate_PID_performance(pid_params):
        ...
        Kp, Ki, Kd = pid_params
        return doublet_test(Kp, Ki, Kd) # Can add reactor params here - no need for global vars
    ...
    if zoom:
        ...
        total_err_zoom_str = str(zoomreturn['best_score'] + ',')
    ...
    str_to_print = Kc_best_str + Ki_best_str + Kd_best_str + total_err_best_str + \
    ...
    #     'best_position': final_result['best_position'],
    #     'best_score': final_result['best_score'],
    #     'expanded_bounds': final_result.get('expanded_bounds'),
    #     'expansion_log': final_result.get('expansion_log', []),
    #     'pso_instance': final_result.get('pso_instance')
    # }
...
def data_gen(ts_data:list) -> None:
    ...
    filepath = 'Documents/bo_pid/pso_testing/vvr_prelim_dataset_PSO_06-24-25.csv'
    ...
    for i in range(len(ts_data)):
        ...
        print(ts_data[i])
        starttime = time.time()
        data_string = run_PID_PSO_test(ts_data[i], -4000, 5100, 105)
        endtime = time.time()
...
# From first entry:
# best score: 721.7379367087075
# best position: 347.18994143 908.11919094   6.09363095
# Seems to line up with previous runs and performs a little better.
